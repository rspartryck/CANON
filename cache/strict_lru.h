...
#include "baseCache.h"
#include "../ssdlogging/mm_log_writer.h"
            ...
            4)  // it will automatically grow
{
  ...
  log_writer = new MemLogWriter(10000);
}
...
inline void StrictLRUCache<TKey, TValue, THash>::delink(ListNode* node) {
  if (WITH_LOG) {
    log_writer->AppendData("LLdelink\n", 9);
  }
...
inline void StrictLRUCache<TKey, TValue, THash>::pushFront(ListNode* node) {
  if (WITH_LOG) {
    log_writer->AppendData("LLpushFront\n", 12);
  }
